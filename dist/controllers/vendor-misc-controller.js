...
export const addValueToVendorMisc = async (req, res, next) => {
    ...
    try {
        ...
        const vendorMisc = await VendorMisc.findOneAndUpdate({ vendorId: vendorId, type }, { $addToSet: { values: { $each: values } } }, { new: true, upsert: true, session });
...
export const removeValue = async (req, res, next) => {
    try {
        ...
        const vendorMisc = await VendorMisc.findOneAndUpdate({ vendor: vendorId, type }, { $pull: { values: { value } } }, { new: true });
        console.debug("Vendor misc updated successfully: ", vendorMisc);
        res.status(200).send({
            success: true,
            message: "Vendor misc updated successfully",
...
export const getVendorMiscValuesByVendor = async (req, res, next) => {
    try {
        const vendorId = req.params.id;
        console.debug("Vendor ID from params: ", vendorId);
